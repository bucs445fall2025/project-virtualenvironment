...
None fully completed, a couple attempted - save_artwork, user_profile, simple_art
...
- User Login/Create Account- Some framework and data schema's set up, can successfully create an account with POST requests, front-end/UI and login not set up though. However, we didn't set this as a goal for the last sprint, rather I found that it goes along well with setting up the storage so I thought why not. 
...
We need to lock in, we will meet up/communicate more frequently as we do have a decently sized plate of work. However,
we have a nice skeleton for our code so we don't expect anything to be TOO challenging, probably time consuming though. 
...
Brendan - Front end logic, multiplayer
Theo - Backend Logic, Multiplayer